...
//            })
            ->editColumn('img_ttd', function ($row) {
                ...
                $btn = '<a href="' . url('dashboard/jenis-penandatangan/visualisasi/' . Hashids::encode($row->id_jenis_ttd)) . '" class="btn btn-sm btn-icon btn-dark waves-effect" title="Detail"><i class="fa fa-signature mr-2"></i> VISUALISASI TTE</a>';
                return $btn;
            })
...
//                'img_ttd' => $img_ttd,
    ...
    public function visualisasi($id)
    {
        $checkData = JenisPenandatangan::find(Hashids::decode($id));
        if (count($checkData) > 0) :
            ...
            return view('dashboard_page.jenis-penandatangan.visualisasi', $data);
        else :
            return redirect(route('jenis-penandatangan'))
                ->with('pesan_status', [
                    'tipe' => 'error',
                    'desc' => 'penandatangan tidak ditemukan',
                    'judul' => 'halaman penandatangan'
                ]);
        endif;
    }
